...
const registerFacultyController = async (req, res) => {
  try {
    ...
    const facultyIdPrefix = `FAC-${currentYear}-`;

    const lastFaculty = await facultyDetails
      .findOne({ facultyId: new RegExp(`^${facultyIdPrefix}`) })
      .sort({ facultyId: -1 });

    let nextNumber = 1;
    if (lastFaculty && lastFaculty.facultyId) {
      const lastNumber = parseInt(lastFaculty.facultyId.split("-").pop());
      nextNumber = lastNumber + 1;
    }

    const facultyId = `${facultyIdPrefix}${String(nextNumber).padStart(3, "0")}`;
