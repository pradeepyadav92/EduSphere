...
const getSubjectController = async (req, res) => {
  try {
    ...
    let subjects = await Subject.find(query).populate("branch").populate("assignedFaculty", "firstName lastName facultyId");
...
const addSubjectController = async (req, res) => {
  const { name, code, branch, semester, credits, assignedFaculty } = req.body;
...
const updateSubjectController = async (req, res) => {
  ...
  if (assignedFaculty !== undefined) updateFields.assignedFaculty = assignedFaculty || null;
